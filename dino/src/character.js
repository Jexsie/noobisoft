...
export class Character {
    ...
    if (this._movement_delay_counter > this._character_meta._movement_delay) {
      ...
      this.set_position(
        applyVelocityToPosition(
          this._character_meta._position,
          this._character_meta._velocity
        )
      );
    }
  }
  ...
  get_layout() {
    return this._character_meta._movements_array[
      this._tick_counter % this._character_meta._movements_array.length
    ];
  }
...
export class CharacterAllocator {
  ...
  get_character() {
    ...
    for (
      let i = 0;
      i < this._allocator_character_array._character_array.length;
      i++
    ) {
      if (RANDOM >= this._allocator_character_array._character_array[i][1]) {
        const RANDOM_CHARACTER = new Character(
          this._allocator_character_array._character_array[i][0].clone()
        );
        this._pending_gap =
          this._min_gap + Math.ceil(Math.random() * this._gap_random_max);
